...
export function validarNumero(
  ...
  if (decimales === 0 && !Number.isInteger(value)) {
 ...
 */
export function validarCamposNumericosProducto(data: any): string | null {
  // Validar precio (debe ser positivo, con 2 decimales)
  if (data.precio !== undefined) {
    const errorPrecio = validarNumero(data.precio, 'precio', 0, undefined, 2);
  ...
  if (data.graduacionEsfera !== undefined) {
    const errorEsfera = validarNumero(data.graduacionEsfera, 'graduación esfera', -20, 20, 2);
  ...
  if (data.tamanoPuente !== undefined) {
    const errorPuente = validarNumero(data.tamanoPuente, 'tamaño puente', 0, 100, 1);
    if (errorPuente) return errorPuente;
  }

  if (data.tamanoAros !== undefined) {
    const errorAros = validarNumero(data.tamanoAros, 'tamaño aros', 0, 100, 1);
    if (errorAros) return errorAros;
  }

  if (data.tamanoVarillas !== undefined) {
    const errorVarillas = validarNumero(data.tamanoVarillas, 'tamaño varillas', 0, 200, 1);
